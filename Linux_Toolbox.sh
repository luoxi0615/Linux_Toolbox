...
check_and_install_pkg() {
    ...
    if ! command -v "$cmd_name" &> /dev/null; then
        echo -e "${YELLOW}检测到命令 '$cmd_name' 未安装，正在尝试安装软件包 '$pkg_name'...${NC}"
        # 识别包管理器
        if command -v apt-get &> /dev/null; then
            ...
            echo -e "${RED}无法识别的包管理器，请手动安装 '$pkg_name'。${NC}"
            return 1
        fi
        # 验证安装是否成功
        if ! command -v "$cmd_name" &> /dev/null; then
             echo -e "${RED}软件包 '$pkg_name' 安装失败，请检查您的包管理器和网络。${NC}"
             return 1
        fi
        echo -e "${GREEN}软件包 '$pkg_name' 安装成功。${NC}"
    fi
    return 0
}
...
show_main_menu() {
    clear
    echo -e "${BLUE}================================================================${NC}"
    echo -e "${GREEN}                    Linux 综合运维工具箱                    ${NC}"
    echo -e "${BLUE}================================================================${NC}"

    # 广告位
    echo -e "${YELLOW}--> 广告位 A: [您的服务器托管、CDN加速、高防IP专家] <--${NC}"
    echo -e "${YELLOW}--> 广告位 B: [专业提供网站建设、小程序开发服务]     <--${NC}"
    echo -e "${YELLOW}--> 广告位 C: [联系我们: contact@yourdomain.com]       <--${NC}"
    echo "----------------------------------------------------------------"

    echo -e "服务器IP: ${GREEN}$SERVER_IP${NC}"
    echo -e "系统运行时间: ${GREEN}$UPTIME_STR${NC}"
    echo -e "当前时间: ${GREEN}$(date '+%Y-%m-%d %H:%M:%S')${NC}"
    echo -e "$(get_greeting) 欢迎使用本工具箱！"
    echo "----------------------------------------------------------------"
    echo " 1. 系统与用户管理"
    echo " 2. 网络与连接管理"
    echo " 3. 磁盘与文件系统"
...
oversubscription_check_menu() {
    clear
    echo -e "${BLUE}--- 主机性能与超售风险评估 ---${NC}"
    echo -e "${YELLOW}此功能通过多维度测试来评估主机资源健康状况。${NC}"
    echo -e "${YELLOW}高CPU Steal、低I/O或不稳定的网络是超售的常见指标。${NC}"
    echo "---------------------------------------------------"
    echo " 1.  开始综合评估 (全面检测)"
...
# 1. 系统与用户管理
system_reboot() {
    read -p "警告：您确定要立即重启服务器吗？ (y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
        echo -e "${RED}服务器正在重启...${NC}"
        sudo reboot
    else
        echo "操作已取消。"
    fi
...
change_current_user_password() {
    ...
    sudo passwd "$(whoami)"
    echo -e "${GREEN}密码修改完成。${NC}"
}

change_hostname_func() {
    read -p "请输入新的服务器主机名: " new_hostname
...
sync_time_func() {
    ...
    echo -e "${GREEN}时区与时间同步完成。当前时间: $(date '+%Y-%m-%d %H:%M:%S')${NC}"
}
...
update_system() {
    ...
    echo -e "${GREEN}系统更新完成。建议重启服务器以应用所有更新。${NC}"
}

change_repo_source() {
    os_id=$(get_os_id)
    echo "正在尝试将包管理器源更换为阿里云镜像..."
    read -p "此操作会覆盖您当前的源列表，确定吗? (y/n): " confirm
    ...
    case "$os_id" in
        ubuntu)
            ...
            sudo apt-get update
            echo -e "${GREEN}Ubuntu 源已更换为阿里云。${NC}"
            ;;
        debian)
            sudo cp /etc/apt/sources.list /etc/apt/sources.list.bak
            ...
            sudo apt-get update
            echo -e "${GREEN}Debian 源已更换为阿里云。${NC}"
            ;;
        centos)
            # 以CentOS 7为例
            if [ -f /etc/yum.repos.d/CentOS-Base.repo ]; then
                ...
                echo -e "${GREEN}CentOS 7 源已更换为阿里云。${NC}"
            else
                echo -e "${RED}未找到 CentOS-Base.repo 文件。此功能可能不适用于您的 CentOS 版本。${NC}"
            fi
            ;;
        *)
            echo -e "${RED}此功能暂不支持您的操作系统: $os_id${NC}"
            ;;
    esac
}
...
create_user_func() {
    ...
    read -p "是否将用户 '$username' 添加到 sudo/wheel 组 (设为管理员)? (y/n): " add_sudo
    if [[ "$add_sudo" == "y" ]]; then
        if command -v usermod &> /dev/null; then
            ...
            echo -e "${GREEN}用户 '$username' 已成功创建并设为管理员。${NC}"
        fi
    else
        echo -e "${GREEN}普通用户 '$username' 创建成功。${NC}"
    fi
}
...
find_large_files() {
    read -p "请输入要扫描的起始目录 (默认: /): " scan_dir
    scan_dir=${scan_dir:-/}
    read -p "请输入要查找的文件数量 (默认: 20): " file_count
...
restart_network_service() {
    ...
    if command -v systemctl &> /dev/null; then
        if systemctl list-units --type=service | grep -q 'NetworkManager'; then
            ...
            echo -e "${RED}未找到标准的网络管理服务。${NC}"
            return
        fi
    elif command -v service &> /dev/null; then
        sudo service network restart || sudo service networking restart
    else
        echo -e "${RED}无法确定如何重启网络服务。${NC}"
        return
    fi
    echo -e "${GREEN}网络服务重启命令已发送。${NC}"
}

set_dns_func() {
    read -p "请输入首选DNS服务器 (例如 8.8.8.8): " dns1
    read -p "请输入备用DNS服务器 (例如 8.8.4.4, 可留空): " dns2
    if [[ -z "$dns1" ]]; then
        echo -e "${RED}首选DNS不能为空。${NC}"
        return
    fi
...
toggle_ping_func() {
    ...
    if [[ "$status" == "1" ]]; then
        sudo sysctl -w net.ipv4.icmp_echo_ignore_all=0 && echo -e "${GREEN}Ping (ICMP Echo) 已启用。${NC}"
    else
        sudo sysctl -w net.ipv4.icmp_echo_ignore_all=1 && echo -e "${GREEN}Ping (ICMP Echo) 已禁用。${NC}"
    fi
}

add_ip_func() {
    echo "注意: 此功能为临时添加IP，重启后会失效。永久配置需修改网卡文件。"
    read -p "请输入要添加的IP地址和子网掩码 (例如 192.168.1.100/24): " ip_cidr
    read -p "请输入网络接口名称 (例如 eth0): " interface
    if [[ -z "$ip_cidr" || -z "$interface" ]]; then
        echo -e "${RED}IP地址和接口名称不能为空。${NC}"
        return
    fi
    sudo ip addr add "$ip_cidr" dev "$interface"
    echo -e "${GREEN}IP地址 '$ip_cidr' 已临时添加到接口 '$interface'。${NC}"
...
query_ip_geo() {
    ...
    curl ipinfo.io
    echo ""
    read -p "是否要查询其他IP地址? (y/n): " query_other
    if [[ "$query_other" == "y" ]]; then
        read -p "请输入要查询的IP地址: " other_ip
        if [[ -n "$other_ip" ]]; then
...
test_udp_connectivity() {
    ...
    if dig @8.8.8.8 google.com +time=2 +tries=1 | grep -q "status: NOERROR"; then
        echo -e "${GREEN}UDP 53 端口连通性正常。${NC}"
    else
        echo -e "${RED}UDP 53 端口连通性异常，可能被防火墙或运营商屏蔽。${NC}"
    fi
}
...
test_mail_ports() {
    ...
    for port in 25 465 587; do
        if ncat -zv -w 2 localhost "$port" &>/dev/null; then
            echo -e "端口 ${GREEN}$port${NC} (localhost): ${GREEN}开放${NC}"
        else
            echo -e "端口 ${RED}$port${NC} (localhost): ${RED}关闭或无响应${NC}"
        fi
    done
}
...
configure_port_forwarding() {
    ...
    echo -e "${GREEN}端口转发规则已添加: 本机端口 $src_port -> ${dst_ip}:${dst_port}${NC}"
}

clear_forwarding_rules() {
    read -p "警告：这将清除所有NAT表中的iptables规则，确定吗? (y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
        sudo iptables -t nat -F
        sudo iptables -t nat -X
        echo -e "${GREEN}所有NAT转发规则已清除。${NC}"
    else
        echo "操作已取消。"
    fi
}
...
format_disk_partition() {
    ...
    lsblk
    echo "--------------------"
    read -p "请输入要格式化的磁盘分区 (例如 /dev/sdb1): " partition
    if [[ ! -b "$partition" ]]; then
        echo -e "${RED}错误: '$partition' 不是一个有效的块设备。${NC}"
        return
    fi
    read -p "请输入要格式化的文件系统类型 (默认: ext4): " fs_type
    fs_type=${fs_type:-ext4}
    read -p "警告：格式化将清除 '$partition' 上的所有数据！您确定吗？ (y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
        sudo mkfs."$fs_type" "$partition"
        echo -e "${GREEN}分区 '$partition' 格式化为 '$fs_type' 完成。${NC}"
    else
        echo "操作已取消。"
    fi
}
...
mount_disk_partition() {
    ...
    lsblk
    echo "--------------------"
    read -p "请输入要挂载的磁盘分区 (例如 /dev/sdb1): " partition
    if [[ ! -b "$partition" ]]; then
        echo -e "${RED}错误: '$partition' 不是一个有效的块设备。${NC}"
        return
    fi
    ...
    echo -e "${GREEN}分区 '$partition' 已成功挂载到 '$mount_point'。${NC}"
    
    read -p "是否要设置开机自动挂载? (y/n): " auto_mount
    if [[ "$auto_mount" == "y" ]]; then
        ...
        if [[ -n "$uuid" ]];then
            ...
            echo -e "${GREEN}已将自动挂载条目添加到 /etc/fstab。${NC}"
        else
            echo -e "${RED}无法获取分区的UUID，自动挂载设置失败。${NC}"
        fi
    fi
}
...
unmount_disk_partition() {
    df -h
    echo "--------------------"
    read -p "请输入要卸载的挂载点或设备 (例如 /data 或 /dev/sdb1): " target
    if [[ -z "$target" ]]; then
    ...
    if grep -q "$target" /etc/fstab; then
        read -p "在 /etc/fstab 中找到了相关条目，是否要移除它? (y/n): " remove_fstab
    ...
    sudo umount "$target"
    echo -e "${GREEN}目标 '$target' 卸载完成。${NC}"
}

repair_xfs_superblock() {
    read -p "请输入要修复的XFS分区 (例如 /dev/sdb1): " partition
    if [[ ! -b "$partition" ]]; then
        echo -e "${RED}错误: '$partition' 不是一个有效的块设备。${NC}"
        return
    fi
...
configure_startup_script() {
    ...
    read -p "是否现在使用 nano 编辑该文件? (y/n): " edit_now
    if [[ "$edit_now" == "y" ]]; then
...
search_keyword_in_files() {
    ...
    read -p "请输入要搜索的目录 (默认: /): " search_dir
...
scp_file_transfer() {
    ...
    read -p "请输入远程服务器用户名: " user
    read -p "请输入远程服务器IP或域名: " host
    
    if [[ "$mode" == "1" ]]; then
        read -p "请输入本地文件路径: " local_path
        read -p "请输入远程服务器目标路径: " remote_path
        scp "$local_path" "${user}@${host}:${remote_path}"
    elif [[ "$mode" == "2" ]]; then
        read -p "请输入远程服务器文件路径: " remote_path
...
enable_nested_virtualization() {
    ...
    if [ -d /sys/module/kvm_intel ]; then
        ...
        echo -e "${GREEN}Intel KVM 嵌套虚拟化已启用。${NC}"
    elif [ -d /sys/module/kvm_amd ]; then
        echo "options kvm_amd nested=1" | sudo tee /etc/modprobe.d/kvm_amd.conf
        sudo modprobe -r kvm_amd
        sudo modprobe kvm_amd
        echo -e "${GREEN}AMD KVM 嵌套虚拟化已启用。${NC}"
    else
        echo -e "${RED}未找到KVM模块，无法启用嵌套虚拟化。${NC}"
    fi
}

configure_swap_space() {
    read -p "请输入要创建的SWAP大小 (例如 2G, 1024M): " swap_size
    if [[ -z "$swap_size" ]]; then
...
toggle_ksm_deduplication() {
    ...
    if systemctl is-active --quiet ksmtuned; then
        ...
        echo -e "${GREEN}KSM内存合并已关闭。${NC}"
    else
        sudo systemctl start ksm
        sudo systemctl start ksmtuned
        echo -e "${GREEN}KSM内存合并已开启。${NC}"
    fi
}

# 5. 软件与服务部署
install_bt_panel() {
    read -p "这将执行宝塔官方安装脚本，确定吗？(y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
        os_id=$(get_os_id)
        case "$os_id" in
            ubuntu)
            ...
            centos)
                ...
                ;;
            *)
                echo -e "${RED}宝塔面板可能不支持您的操作系统: $os_id${NC}"
                ;;
        esac
    else
        echo "操作已取消。"
...
manage_docker() {
    ...
    case $choice in
        1) sudo docker ps ;;
        2) sudo docker ps -a ;;
        3) read -p "请输入容器ID或名称: " id; sudo docker start "$id" ;;
        4) read -p "请输入容器ID或名称: " id; sudo docker stop "$id" ;;
        5) read -p "请输入容器ID或名称: " id; sudo docker logs -f "$id" ;;
        6) read -p "请输入容器ID或名称: " id; sudo docker rm "$id" ;;
        q) return ;;
        *) echo "无效选择" ;;
    esac
}
...
install_dev_tools() {
    ...
    case "$os_id" in
        ...
        centos|rhel|almalinux|rocky)
            ...
            ;;
        *)
            echo -e "${RED}无法为您的操作系统 '$os_id' 自动安装开发工具。${NC}"
            return
            ;;
    esac
    echo -e "${GREEN}开发工具包安装完成。${NC}"
}

install_kangle() {
    read -p "这将执行彩虹Kangle一键安装脚本，确定吗? (y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
...
install_bt_waf() {
    read -p "这将执行宝塔云WAF安装脚本，确定吗? (y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
...
# 6. 主机性能与超售风险评估
check_cpu_steal() {
    echo -e "\n${BLUE}--- 1. CPU Steal Time 评估 ---${NC}"
    if ! check_and_install_pkg "sysstat" "sar"; then return; fi
    echo "正在收集CPU数据（5秒）..."
    local steal_time=$(sar -u 1 5 | tail -n 1 | awk '{print $6}')
    echo "CPU Steal Time: ${YELLOW}${steal_time}%${NC}"
    if (( $(echo "$steal_time > 10.0" | bc -l) )); then
        echo -e "${RED}风险评估: 高风险。CPU Steal Time 显著偏高，表示主机CPU资源严重拥塞，是超售的强烈信号。${NC}"
    elif (( $(echo "$steal_time > 5.0" | bc -l) )); then
        echo -e "${YELLOW}风险评估: 中等风险。CPU Steal Time 有些偏高，可能存在资源争抢。${NC}"
    else
        echo -e "${GREEN}风险评估: 低风险。CPU Steal Time 处于正常水平。${NC}"
    fi
}

check_disk_io() {
    echo -e "\n${BLUE}--- 2. 磁盘 I/O 性能评估 ---${NC}"
    ...
    rm -f test_io_file
    echo "磁盘写入速度: ${YELLOW}${io_speed}${NC}"
    local speed_val=$(echo "$io_speed" | awk '{print $1}')
    if (( $(echo "$speed_val < 50" | bc -l) )); then
        echo -e "${RED}性能评估: 较差。磁盘性能可能受限，可能是HDD或I/O限制严格的虚拟化平台。${NC}"
    elif (( $(echo "$speed_val < 200" | bc -l) )); then
        echo -e "${YELLOW}性能评估: 普通。性能尚可，类似于普通SATA SSD或性能一般的云盘。${NC}"
    else
        echo -e "${GREEN}性能评估: 良好。磁盘性能表现出色，接近现代NVMe SSD标准。${NC}"
    fi
}
...
check_cpu_benchmark() {
    ...
    echo "单核CPU性能 (events per second): ${YELLOW}${cpu_events}${NC}"
    if (( $(echo "$cpu_events < 500" | bc -l) )); then
        echo -e "${RED}性能评估: 较低。CPU单核性能较弱，可能为低主频或受限制的vCPU。${NC}"
    elif (( $(echo "$cpu_events < 1500" | bc -l) )); then
        echo -e "${YELLOW}性能评估: 中等。性能水平一般，适用于中低负载应用。${NC}"
    else
        echo -e "${GREEN}性能评估: 良好。CPU单核性能强劲。${NC}"
    fi
}
...
run_comprehensive_assessment() {
    clear
    echo -e "${BLUE}====================================================${NC}"
    echo -e "${GREEN}        开始执行主机性能与超售风险综合评估        ${NC}"
    echo -e "${BLUE}====================================================${NC}"
    check_cpu_steal
    check_disk_io
    check_network_quality
    ...
    echo "核心关注 ${YELLOW}CPU Steal Time${NC}，该值是判断CPU超售最直接的参考。"
}

# 7. 系统安全与日志审计
run_security_audit() {
    clear
    echo -e "${BLUE}--- 基础安全审计 ---${NC}"
    # 检查root登录
    if grep -q "^PermitRootLogin yes" /etc/ssh/sshd_config; then
        echo -e "${RED}[高风险] SSH允许root用户直接密码登录。建议修改为 'PermitRootLogin prohibit-password'。${NC}"
    else
        echo -e "${GREEN}[OK] SSH不允许root用户密码登录。${NC}"
    fi

    # 检查开放端口
    echo -e "\n${BLUE}正在监听的TCP/UDP端口:${NC}"
    if command -v ss &> /dev/null; then
        sudo ss -tuln
    elif command -v netstat &> /dev/null; then
        check_and_install_pkg "net-tools" "netstat" && sudo netstat -tuln
    else
        echo -e "${YELLOW}无法找到 ss 或 netstat 命令。${NC}"
    fi
    ...
    if command -v sestatus &> /dev/null; then
        ...
        if [[ $selinux_status == "enabled" ]]; then
             echo -e "\n${YELLOW}[注意] SELinux 当前状态为: $selinux_status。某些应用可能需要特定策略。${NC}"
        else
             echo -e "\n${GREEN}[OK] SELinux 当前状态为: $selinux_status。${NC}"
        fi
    fi
}
...
manage_ssh_service() {
    if systemctl is-active --quiet sshd || systemctl is-active --quiet ssh; then
        read -p "SSH服务当前正在运行，是否要停止并禁用它? (y/n): " confirm
        if [[ "$confirm" == "y" ]]; then
            ...
            echo -e "${GREEN}SSH服务已停止并禁用。${NC}"
        fi
    else
        read -p "SSH服务当前已停止，是否要启动并启用它? (y/n): " confirm
        if [[ "$confirm" == "y" ]]; then
...
change_ssh_port() {
    ...
    echo -e "${GREEN}SSH端口已在配置文件中修改为 $new_port。${NC}"
    echo "请注意：如果启用了防火墙，您需要手动放行新端口 $new_port。"
    read -p "是否现在重启SSH服务以应用更改? (y/n): " restart_ssh
...
disable_selinux_permissive() {
    ...
    if [[ "$(sestatus | awk '{print $3}')" == "disabled" ]]; then
        echo -e "${GREEN}SELinux 已经是 disabled 状态。${NC}"
        return
    fi
    read -p "警告：这将把SELinux设为 permissive (宽容) 模式，确定吗? (y/n): " confirm
    if [[ "$confirm" == "y" ]]; then
        sudo setenforce 0
        sudo sed -i 's/^SELINUX=enforcing/SELINUX=permissive/' /etc/selinux/config
        echo -e "${GREEN}SELinux 已临时并永久设置为 permissive 模式。${NC}"
    fi
}
...
manage_shell_history() {
    ...
    if [[ "$choice" == "1" ]]; then
        ...
        read -p "警告：这将清空您当前会话的历史命令，确定吗? (y/n): " confirm
        if [[ "$confirm" == "y" ]]; then
            history -c
            echo "" > ~/.bash_history
            echo -e "${GREEN}历史命令已清空。${NC}"
        fi
    fi
}
...
main() {
    while true; do
        show_main_menu
        read -p "请输入选项 [1-7, q]: " main_choice
        case $main_choice in
            1) # 系统管理
                while true; do system_menu; read -p "请输入选项: " c; case $c in
                    ...
                    q) break ;; *) echo -e "${RED}无效选项!${NC}"; sleep 1 ;; esac; done ;;
            2) # 网络管理
                while true; do network_menu; read -p "请输入选项: " c; case $c in
                    ...
                    q) break ;; *) echo -e "${RED}无效选项!${NC}"; sleep 1 ;; esac; done ;;
            3) # 磁盘管理
                while true; do disk_menu; read -p "请输入选项: " c; case $c in
                    ...
                    q) break ;; *) echo -e "${RED}无效选项!${NC}"; sleep 1 ;; esac; done ;;
            4) # 虚拟化管理
                while true; do virtualization_menu; read -p "请输入选项: " c; case $c in
                    ...
                    q) break ;; *) echo -e "${RED}无效选项!${NC}"; sleep 1 ;; esac; done ;;
            5) # 软件部署
                 while true; do software_menu; read -p "请输入选项: " c; case $c in
                    ...
                    q) break ;; *) echo -e "${RED}无效选项!${NC}"; sleep 1 ;; esac; done ;;
            6) # 性能评估
                while true; do oversubscription_check_menu; read -p "请输入选项: " c; case $c in
                    ...
                    q) break ;; *) echo -e "${RED}无效选项!${NC}"; sleep 1 ;; esac; done ;;
            7) # 安全审计
                 while true; do security_menu; read -p "请输入选项: " c; case $c in
